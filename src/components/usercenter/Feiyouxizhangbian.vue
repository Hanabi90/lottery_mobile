<template>
    <div class="bethistory">
        ...
        <van-button
            ...
            loading-text="查询中..."
            @click.prevent="getbankreporthistory(false)"
        >查询</van-button>
        <van-list
            v-model="listLoading"
            :finished="finished"
        ...
        >
            <van-collapse v-model="activeNames" accordion>
                ...
                >
                    <div slot="title" :class="[item.cntitle.includes('-')?'minus':'plus']">{{item.cntitle}}</div>
                    <Cell>
                        用户名:
                    ...
                    <Cell>
                        账变编号:
                        <span class="cell_span">{{item.orderno}}</span>
                    </Cell>
                    <Cell>
                        收入:
                        <span class="cell_span minus" v-if="item.operations>0">{{item.amount}}</span>
                    </Cell>
                    <Cell>
                        支出:
                        <span class="cell_span plus" v-if="item.operations==0">{{item.amount}}</span>
                    </Cell>
                    <Cell>
                        余额:
                        <span class="cell_span">{{item.money}}</span>
                    </Cell>
                    <Cell>
                        状态:
                        <span class="cell_span" v-if="item.transferstatus==1||item.transferstatus==3">失败</span>
                        <span class="cell_span" v-else>成功</span>
                    </Cell>
                    <Cell>
                        备注:
...
export default {
    ...
    methods: {
        onLoad() {
            ...
            this.getbankreporthistory(true)
            this.listLoading = false
        },
        getbankreporthistory(flag) {
