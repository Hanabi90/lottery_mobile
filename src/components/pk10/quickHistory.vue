            ...
            <van-collapse v-model="activeNames">
                ...
                <van-collapse-item size="large" title="未读信息" name="3" icon="comment" class="msg_xiala">
                    <van-swipe-cell v-for="(msgItem, index) in msgArr.page_data" :key="index + msgItem.id" :right-width="60" :left-width="60"  :on-close="onClose">
                        ...
                        <van-cell @click="onOpen(msgItem,index)" :border="false"  :value="msgItem.title">
                            <template slot="title">
        ...
        <div class="chatBox" v-show="isShowChatBox">
            ...
            <div v-if="isshowmsgbox">
                ...
                <div class="msg_content">
                    <span>发送人</span><span>{{currentMsgItem.mes.title}}</span>
                </div>
                <div class="msg_time">
                    <span>时间</span><span>{{currentMsgItem.mes.sendtime}}</span>
                </div>
                <div class="msg_content">
                    <span>标题</span><span>{{currentMsgItem.mes.subject}}</span>
                </div>
                <div class="msg_title">
                    <span>内容</span><span>{{currentMsgItem.mes.content}}</span>
                </div>
            </div>
            <div class="buttons_reply">
                <van-button type="info" @click="reply()">回复</van-button>
                <van-button type="danger" @click="delmsg()">删除</van-button>
            </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        onOpen(msgItem,index){
            getmessagecontent({entry:msgItem.entry}).then((res)=>{
                this.currentMsgItem = res.data
                this.chatCtrl({username:res.data.receiver.username},true,false)
        ...
        delmsg(){
            deletemessage({flag:'receiveuser',msgid:[this.currentmsgentry]}).then((res)=>{
                if(res.code==0){
                    Toast.success("删除成功")
                    this.chatCtrl('',false,false)
                    this.msgArr['page_data'].splice(this.currentIndex,1)
                }
            })
        },
        sendmsg() {
        ...
        getmessage() {
            getmessage({ flag: 'recieve', pn: 13 }).then(res => {
                if(res.code==0){
